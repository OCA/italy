...
class StockDeliveryNote(models.Model):
    ...
    volume_uom_id = fields.Many2one('uom.uom',
                                    ...
                                    states=DONE_READONLY_STATE)
    gross_weight = fields.Float(string="Gross weight", store=True, readonly=False, compute='_compute_weights',
                                states=DONE_READONLY_STATE)
    gross_weight_uom_id = fields.Many2one('uom.uom',
                                          ...
                                          states=DONE_READONLY_STATE)
    net_weight = fields.Float(string="Net weight", store=True, readonly=False, compute='_compute_weights',
                              states=DONE_READONLY_STATE)
        ...
        for note in self:
            if note.picking_ids:
                ...
                for pick in note.picking_ids:
                    gross_weight += weight_uom._compute_quantity(pick.shipping_weight, note.gross_weight_uom_id)
                    net_weight += weight_uom._compute_quantity(pick.shipping_weight, note.net_weight_uom_id)
    ...
    def _create_detail_lines(self, move_ids):
        ...
        lines_vals = \
            self.env['stock.delivery.note.line']._prepare_detail_lines(moves)
    ...
    def update_detail_lines(self):
        for note in self:
            ...
            move_ids_to_create = [l for l in pickings_move_ids if
                                  l not in lines_move_ids]
            move_ids_to_delete = [l for l in lines_move_ids if
                                  l not in pickings_move_ids]
...
class StockDeliveryNoteLine(models.Model):
    ...
    def sync_invoice_status(self):
        for line in self.filtered(lambda l: l.sale_line_id):
            ...
            line.invoice_status = DOMAIN_INVOICE_STATUSES[
                1] if invoice_status == 'upselling' else invoice_status
